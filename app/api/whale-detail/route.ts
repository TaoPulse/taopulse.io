...
export async function GET(req: Request) {
  ...
  try {
    ...
    const delegations = (delegationsRes.data ?? []).map((d: any) => {
      const coldkey =
        typeof d.coldkey === "object" ? (d.coldkey?.ss58 ?? "") : (d.coldkey ?? "");
      const hotkey =
        typeof d.hotkey === "object" ? (d.hotkey?.ss58 ?? "") : (d.hotkey ?? "");
      return {
        action: d.action ?? (d.delegate_added !== undefined ? "STAKE" : "UNKNOWN"),
    ...
    // Determine last active from most recent transfer or delegation
    const allTimestamps = [
    ...
    // Recently unstaked flag: any UNDELEGATE in last 7 days
